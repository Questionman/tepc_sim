...
*/
 ...
 // Set Inner Propane as scoring volume
  ...
  fScoringVolume_phys = physicsInnerPropane;
  //
  //always return the physical World
  //
  fWorldVolume_phys = physWorld;
  return physWorld;
}
...
G4VIStore* B1DetectorConstruction::CreateImportanceStore()
{
  G4cout << " B1DetectorConstruction:: Creating Importance Store " << G4endl;
 /*f (!fPhysicalVolumeVector.size())
  {
    G4Exception("B1DetectorConstruction::CreateImportanceStore"
               ,"exampleB1",RunMustBeAborted
               ,"no physical volumes created yet!");
  }

  fWorldVolume_phys = fPhysicalVolumeVector[0]; */

  // creating and filling the importance store
  
  G4IStore *istore = G4IStore::GetInstance();
  G4double imp =10;
  istore->AddImportanceGeometryCell(1,*GetWorldVolume());
  printf("here to go\n");
  G4cout << "Going to assign importance: " << imp << ", to volume: " 
             << fScoringVolume_phys->GetName() << G4endl;
  istore->AddImportanceGeometryCell(imp, *fScoringVolume_phys);
